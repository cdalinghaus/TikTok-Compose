...
object CommentDataSource {
    ...
    suspend fun fetchComment(videoId: String): CommentList {
        Log.d("COMMENTS", "Loading comments for $videoId")
        ...
        val response = statisticsApi.getComments(videoId)

        if (!response.isSuccessful) {
            Log.d("COMMENTS", "PROBLEM")
            // Here you might want to handle the error, possibly by emitting an empty list or a specific error.
        }
        Log.d("COMMENTS", response.body()!!.data.toString())
